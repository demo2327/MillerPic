...
| --- | --- | --- | --- | --- | --- |
| `CKV_AWS_50` | Lambda functions in `infrastructure/lambda.tf` | X-Ray tracing deferred due recurring ingestion/storage cost for current family-scale workload | CloudWatch logs and alarms, Lambda DLQ, API error/throttle alarms | MillerPic Platform Team | 2026-10-02 (reviewed 2026-07-02, no change) |
| `CKV_AWS_18` | S3 buckets `photos` and `terraform_state` | S3 access logging deferred to avoid additional storage/request cost this budget cycle | CloudTrail audit logs, S3 public access block, versioning, IAM least privilege | MillerPic Platform Team | 2026-10-02 (reviewed 2026-07-02, no change) |
| `CKV_AWS_144` | S3 buckets `photos` and `terraform_state` | Cross-region replication deferred due additional data-transfer and storage cost | Versioning enabled, least-privilege IAM, periodic backup/recovery review | MillerPic Platform Team | 2026-03-16 |
| `CKV2_AWS_62` | S3 buckets `photos` and `terraform_state` | S3 object-level event notifications intentionally omitted (no security value for current threat model; would add non-actionable event/cost noise) | Explicit deny bucket policies restricting object access to approved principals only, S3 public access block, KMS encryption, CloudTrail audit logs, versioning | MillerPic Platform Team | 2026-03-16 |
...
| `CKV_AWS_273` | `aws_iam_user.terraform_deployer` in `infrastructure/bootstrap/main.tf` | Terraform deployer intentionally remains IAM-user based for controlled manual key lifecycle management | Manual access-key rotation/revocation process, restricted IAM permissions, monthly security review | MillerPic Platform Team | 2026-03-16 |
